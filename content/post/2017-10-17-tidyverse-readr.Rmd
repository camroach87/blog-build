...
Make a habit of storing all your files in the project directory and using relative paths. This way if you ever need to move your code you can simply copy the entire project directory, open the `.Rproj` file and run the code without path issues.
...
Our example data set contains a few entries with leading or trailing whitespace characters. Conveniently, `read_csv` will remove any trailing or leading whitespace characters by default, but this behaviour can be disabled if need be by setting `trim_ws = FALSE`.
